...
export default async function LandingPage({
  ...
  return (
    <main className="flex flex-col gap-24 py-12 md:py-24 overflow-hidden">
      ...
      <section className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 text-center space-y-10 pt-4 sm:pt-6">
        ...
        <div className="pt-8 border-t border-[var(--color-border-subtle)]/60 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-4xl mx-auto text-left">
          <div className="p-3">
          ...
          <div className="p-3">
            <div className="text-2xl font-bold text-cyan-400 font-display">168s</div>
            <div className="text-xs text-[var(--color-text-secondary)] mt-0.5">
              {isTr ? "Maksimum İlan Canlılığı" : "Freshness Lifecycle"}
            </div>
          </div>
          <div className="p-3">
            <div className="text-2xl font-bold text-indigo-400 font-display">AES-256</div>
            <div className="text-xs text-[var(--color-text-secondary)] mt-0.5">
              {isTr ? "Birebir Gizli Teklif" : "Encrypted Offers"}
            </div>
          </div>
          <div className="p-3">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-blue-400">
            ...
            <span>{isTr ? "Sistem Mimarisi" : "Platform Mechanics"}</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-[var(--color-text-primary)]">
            {isTr
              ? "Geleneksel Platformların Kusurlarını Çözen 3 Temel Prensip"
              : "3 Core Principles Eliminating Freelance Market Defects"}
          </h2>
          <p className="text-sm sm:text-base text-[var(--color-text-secondary)]">
            {isTr
              ? "Her bir sistem bileşeni şeffaflığı, gizliliği ve doğrudan ticari bağımsızlığı korumak için matematiksel ve operasyonel kurallarla tasarlandı."
              : "Engineered specifically to guarantee privacy, freshness, and direct commercial autonomy between engineers and clients."}
          </p>
        </div>
