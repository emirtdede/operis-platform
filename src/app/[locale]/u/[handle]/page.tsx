...
import { MapPin, ExternalLink, CheckCircle2, ArrowLeft } from "lucide-react";
...
export async function generateMetadata({
  ...
  const description = profile.about ?? (isTr
  ...
  return {
    ...
    description,
    alternates: {
      canonical: isTr ? `/tr/profil/${handle}` : `/en/profile/${handle}`,
      languages: {
        tr: `/tr/profil/${handle}`,
        en: `/en/profile/${handle}`,
      },
    },
    openGraph: {
      title,
      description,
      url: isTr ? `/tr/profil/${handle}` : `/en/profile/${handle}`,
      siteName: "Operis",
...
export default async function PublicProfilePage({
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8 space-y-8">
      ...
      <article className="relative overflow-hidden rounded-3xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/70 backdrop-blur-xl p-6 sm:p-10 space-y-6 shadow-xl">
        ...
        <header className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div className="flex items-center gap-5">
            <AvatarInitials name={profile.displayName} size="lg" />
            <div className="space-y-1">
              ...
              {profile.location && (
                <div className="text-xs text-[var(--color-text-secondary)] pt-1 flex items-center gap-1">
                  <MapPin className="h-3.5 w-3.5 text-[var(--color-text-tertiary)]" aria-hidden="true" />
        ...
        <div className="border-t border-[var(--color-border-subtle)] pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="space-y-0.5 text-center sm:text-left">
            <span className="text-sm font-bold text-[var(--color-text-primary)]">
              {isTr ? "Bu uzmanla çalışmak mı istiyorsunuz?" : "Want to collaborate with this specialist?"}
            </span>
          ...
          <Link href={isTr ? "/tr/ilanlar/yeni" : "/en/listings/new"} className="shrink-0 w-full sm:w-auto">
      ...
      <section className="rounded-3xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/70 backdrop-blur-xl p-6 sm:p-8 space-y-6 shadow-sm" aria-label={isTr ? "Tamamlanan Projeler" : "Completed Projects"}>
        <div className="flex items-center justify-between border-b border-[var(--color-border-subtle)] pb-4">
          <div>
            <h2 className="text-lg font-bold text-[var(--color-text-primary)] flex items-center gap-2">
              <CheckCircle2 className="h-5 w-5 text-emerald-500" aria-hidden="true" />
              <span>{isTr ? "Doğrulanmış Tamamlanan Projeler" : "Verified Completed Projects"}</span>
            </h2>
        ...
        {(!profile.completedWork || profile.completedWork.length === 0) ? (
          <EmptyState
            title={
              isTr
                ? "Henüz tamamlanmış proje kaydı bulunmuyor"
                : "No completed project records yet"
            }
            description={
              isTr
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-[var(--color-text-secondary)] pt-1">
                    <span>{isTr ? "Tamamlanma:" : "Completed:"} {formattedDate}</span>
